...
function px(b){return parseFloat((b.price||'0').replace(/,/g,'').replace(/[^\d.]/g,''))||0}
  ...
  catalog.querySelectorAll('.bcard').forEach(el=>{
    ...
    if(addBtn && !addBtn.classList.contains('sold')){
      addBtn.addEventListener('click', e=>{
        e.stopPropagation();
        const book=allBooks.find(b=>(b.isbn||'').replace(/[\s\-]/g,'')===(isbn||'').replace(/[\s\-]/g,'') || b.title===isbn);
...
function toggleCart(book, btn, isDetail=false) {
  if (!book) { console.warn('toggleCart: book is undefined'); return; }
  const key = book.isbn || book.title;
  if (cart[key]) {
    delete cart[key];
...
function renderCart() {
  ...
  const DFEE=50,FABOVE=499,dfree=totalPrice>=FABOVE,del=dfree?0:DFEE,grand=totalPrice+del;
  $('cartSumItems').textContent=totalItems;
  $('cartSumTotal').textContent=totalPrice?`₹${totalPrice}`:'—';
  $('cartDelivery').innerHTML=dfree?'<span class="delivery-free">FREE 🎉</span>':`₹${DFEE}`;
  $('cartGrandTotal').textContent=grand?`₹${grand}`:'—';
  const tip=$('freeTip');
  if(!dfree&&totalPrice>0){tip.style.display='block';$('freeAmt').textContent=`₹${FABOVE-totalPrice}`}
...
function openOrderForm() {
  ...
  summaryHtml += `
    <div class="om-total-row" style="border-top:1px solid var(--border);">
      <span style="font-weight:500;color:var(--text2)">Subtotal</span><span style="color:var(--text)">₹${subtotal}</span>
    </div>
    <div class="om-total-row" style="border-top:1px solid var(--border);">
      <span style="font-weight:500;color:var(--text2)">Delivery</span>
      <span style="color:${delivery===0?'var(--green)':'var(--text2)'};font-weight:700">${delivery===0?'FREE 🎉':'₹50'}</span>
    </div>
...
document.addEventListener('keydown', e=>{
  if(e.key==='Escape'){
    if($('orderSuccessOverlay').classList.contains('show')){ closeSuccessScreen(); }
    else if($('orderOverlay').classList.contains('show')){ closeOrderForm(); }
    else if($('cartOverlay').classList.contains('open')){ $('cartOverlay').classList.remove('open'); document.body.style.overflow=''; }
